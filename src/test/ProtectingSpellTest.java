...
class ProtectingSpellTest {
    ...
    @Test
    void constructor() {
        Assertions.assertThrows(IllegalArgumentException.class, () -> new ProtectingSpell("a", 3, MagicLevel.NOOB, null));
        Assertions.assertThrows(IllegalArgumentException.class, () -> new ProtectingSpell("a", 3, MagicLevel.NOOB, new HashSet<>()));
        Assertions.assertDoesNotThrow(() -> new ProtectingSpell("a", 3, MagicLevel.NOOB, new HashSet<>(Arrays.asList(fireball, poison))));
    }
    ...
    void additionalOutputString() {
        var tmp = new HashSet<>(Arrays.asList(fireball, poison));
        Assertions.assertEquals("; protects against " + tmp, SP.additionalOutputString());
    }
}