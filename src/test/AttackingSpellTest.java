...
import a12226166.AttackingSpell;
...
class AttackingSpellTest {
    ...
    @Test
    void constructor() {
        Assertions.assertThrows(IllegalArgumentException.class, () -> new AttackingSpell("a", 3, MagicLevel.NOOB, false, false, -1));
        Assertions.assertDoesNotThrow(() -> new AttackingSpell("a", 3, MagicLevel.NOOB, false, false, 0));
        Assertions.assertDoesNotThrow(() -> new AttackingSpell("a", 3, MagicLevel.NOOB, false, false, 101));
        Assertions.assertThrows(IllegalArgumentException.class, () -> new AttackingSpell("a", 3, MagicLevel.NOOB, false, true, -1));
        Assertions.assertThrows(IllegalArgumentException.class, () -> new AttackingSpell("a", 3, MagicLevel.NOOB, false, true, 101));
        Assertions.assertDoesNotThrow(() -> new AttackingSpell("a", 3, MagicLevel.NOOB, false, true, 100));
        Assertions.assertDoesNotThrow(() -> new AttackingSpell("a", 3, MagicLevel.NOOB, false, true, 0));
    }

    @Test
    void doEffect() {
        var protector = new Wizard("Protector", MagicLevel.STUDENT, 3, 3, 999, 999,
                999, new HashSet<>(), new HashSet<>(Arrays.asList(fireball, poison, extraction, timedextraction)),
                999, new HashSet<>());
        var die = new Wizard("Die", MagicLevel.STUDENT, 6, 3, 999, 999,
                999, new HashSet<>(), new HashSet<>(), 999, new HashSet<>());
        var manadude = new Wizard("Mana Dude", MagicLevel.NOOB, 999, 999, 50, 50,
                999, new HashSet<>(), new HashSet<>(), 999, new HashSet<>());
